...
function normalizePackageItems(items = [], legacyPackageName = '') {
  ...
  return String(legacyPackageName || '')
    ...
    .filter(Boolean)
    .map((token) => {
      const isCompleted = /^(\+|\[x\]|x\s+|òÜÓ\s+|òÜÔ\s+)/i.test(token);
      const normalizedName = token
        .replace(/^(\+|\-|\[x\]|\[\s\]|x\s+|òÜÓ\s+|òÜÔ\s+)/i, '')
        .trim();
      return {
...
function getMaterialRequestItemDisplayName(item = {}) {
  ...
  if (normalizedName && normalizedName.toLowerCase() !== 'TÄ¦-TÂ¦-') return normalizedName;
  ...
  return normalizedName || '¦ä¦-TÂ¦-';
}

function getTelegramMaterialRequestErrorMessage(error, fallbackMessage = '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦+¦-¦-¦-¦-¦¬TÂTÌ ¦¬¦-TÏ¦-¦¦TÃ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬.') {
  const rawMessage = String(error?.message || '').trim();
  if (rawMessage && !/^error\.?$/i.test(rawMessage)) {
    return rawMessage;
  }
  return `${fallbackMessage} ¦ßTÀ¦-¦-¦¦TÀTÌTÂ¦¦, TÇTÂ¦- TÁ¦-TÂTÀTÃ¦+¦-¦¬¦¦ ¦-¦-TÂ¦-TÀ¦¬¦¬¦-¦-¦-¦- ¦- Telegram-¦-¦-TÂ¦¦, QR ¦-TÂ¦¦TÀTËTÂ ¦-¦- TÁTÃTÉ¦¦TÁTÂ¦-TÃTÎTÉ¦¦¦¦ ¦¬¦¬¦+¦¦¦¬¦¬¦¦ ¦¬ TÁ¦¦TÀ¦-¦¦TÀ ¦+¦-TÁTÂTÃ¦¬¦¦¦- ¦+¦¬TÏ TÁ¦-TÅTÀ¦-¦-¦¦¦-¦¬TÏ ¦¬¦-¦¦¦-¦¬¦-.`;
}
...
function getAttachmentKindLabel(attachment = {}) {
  if (isLinkAttachment(attachment)) return '¦áTÁTË¦¬¦¦¦-';
  ...
  if (isImageAttachment(attachment)) return '¦Ø¦¬¦-¦-TÀ¦-¦¦¦¦¦-¦¬¦¦';
  return '¦ä¦-¦¦¦¬';
}
...
function formatAttachmentSize(size) {
  ...
  if (numericSize >= 1024 * 1024) {
    return `${(numericSize / (1024 * 1024)).toFixed(1)} ¦Ü¦Ñ`;
  }
  return `${Math.max(1, Math.round(numericSize / 1024))} ¦Ú¦Ñ`;
}
...
function getTelegramReadOnlySection(item, sectionKey) {
  ...
  if (sectionKey === 'orderCard') {
    return {
      key: 'orderCard',
      title: '¦Ú¦-TÀTÂ¦-TÇ¦¦¦- ¦¬¦-¦¦¦-¦¬¦-',
      emptyText: '¦ä¦-¦¦¦¬TË ¦¦¦-TÀTÂ¦-TÇ¦¦¦¬ ¦¬¦-¦¦¦-¦¬¦- ¦-¦¦ ¦¬TÀ¦¬¦¦TÀ¦¦¦¬¦¬¦¦¦-TË.',
      text: '',
...
function OrderDetail() {
  ...
  useGlobalErrorEffect(sessionError, '¦ÞTÈ¦¬¦-¦¦¦- ¦-¦¬TÀ¦¦¦+¦¦¦¬¦¦¦-¦¬TÏ ¦¬TÀ¦-TÄ¦¬¦¬TÏ ¦- Telegram.');
  useGlobalErrorEffect(scanActivationError, '¦ÞTÈ¦¬¦-¦¦¦- ¦¬TÀ¦¬¦-TÏTÂ¦¬TÏ ¦¬¦¬¦+¦¦¦¬¦¬TÏ ¦- TÀ¦-¦-¦-TÂTÃ.');
  useGlobalErrorEffect(stageError, '¦ÞTÈ¦¬¦-¦¦¦- ¦-TÂ¦-¦¦TÂ¦¦¦¬ TÍTÂ¦-¦¬¦-.');
  useGlobalErrorEffect(packageError, '¦ÞTÈ¦¬¦-¦¦¦- ¦¦¦-¦-¦¬¦¬¦¦¦¦TÂ¦-TÆ¦¬¦¬.');
  useGlobalErrorEffect(materialRequestError, '¦ÞTÈ¦¬¦-¦¦¦- ¦¬¦-TÏ¦-¦-¦¦ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬.');
  useGlobalErrorEffect(telegramActionError, '¦ÞTÈ¦¬¦-¦¦¦- ¦+¦¦¦¦TÁTÂ¦-¦¬TÏ ¦- Telegram.');
  ...
  const fetchOrderStageLegendConfig = useCallback(async () => {
    try {
      ...
      const data = await parseJsonSafely(res);
      if (!res.ok) {
        throw new Error(data?.message || '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦¬¦-¦¦TÀTÃ¦¬¦¬TÂTÌ TÍTÂ¦-¦¬TË ¦¬TÀ¦-¦¬¦¬¦-¦-¦+TÁTÂ¦-¦-.');
      }
  ...
  const loadTelegramEmployeeSession = useCallback(() => {
    ...
    if (!hasTelegramAuthPayload && !currentSessionToken) {
      ...
      setSessionError('¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦¬¦-¦+TÂ¦-¦¦TÀ¦+¦¬TÂTÌ ¦-¦-TÈ ¦+¦-TÁTÂTÃ¦¬. ¦ÞTÂ¦¦TÀ¦-¦¦TÂ¦¦ ¦¬¦-¦¦¦-¦¬ ¦¬¦-¦-¦-¦-¦- TÇ¦¦TÀ¦¦¦¬ ¦¦¦-¦-¦¬¦¦TÃ ¦- ¦-¦-TÂ¦¦.');
      return;
    }
    setSessionLoading(true);
    setSessionError('');
    ...
    const resolveSession = async (sessionTokenOverride = currentSessionToken) => {
      ...
      const data = await parseJsonSafely(res);
      if (!res.ok) {
        throw new Error(data?.message || '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-¦¬TÀ¦¦¦+¦¦¦¬¦¬TÂTÌ ¦-¦-TÈ ¦¬TÀ¦-TÄ¦¬¦¬TÌ.');
      }
      return data;
    };
    ...
    return resolveSession()
      ...
      .catch(error => {
        setTelegramEmployee(null);
        setSessionError(toUserErrorMessage(error, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-¦¬TÀ¦¦¦+¦¦¦¬¦¬TÂTÌ ¦-¦-TÈ ¦¬TÀ¦-TÄ¦¬¦¬TÌ.'));
      })
  ...
  const calcDuration = (start, end) => {
    if (!start || !end) return 'òÀÔ';
    const s = new Date(start);
    const e = new Date(end);
    const diff = Math.round((e - s) / (1000 * 60 * 60 * 24));
    return diff >= 0 ? diff + ' ¦+¦-.' : 'òÀÔ';
  };
  ...
  const detailItems = order ? [
    { label: '¦Ý¦-¦-¦¦TÀ ¦¬¦-¦¦¦-¦¬¦-', value: order.orderNumber || 'òÀÔ' },
    { label: '¦×¦-¦¦¦-¦¬TÇ¦¬¦¦', value: order.customer || 'òÀÔ' },
    { label: '¦Ø¦¬¦+¦¦¦¬¦¬¦¦', value: selectedItem?.name || primaryItem?.name || 'òÀÔ' },
    { label: 'òÄÖ ¦¬¦¬¦+¦¦¦¬¦¬TÏ ¦- ¦¬¦-¦¦¦-¦¬¦¦', value: selectedItem?.itemNumber || 'òÀÔ' },
    { label: '¦ß¦-¦-¦¦TÉ¦¦¦-¦¬¦¦', value: selectedItem?.room || 'òÀÔ' },
    { label: '¦Ú¦-¦¬¦¬TÇ¦¦TÁTÂ¦-¦-', value: selectedItem?.quantity || primaryItem?.quantity || 1 },
  ] : [];
  ...
  useEffect(() => {
    ...
    const activateItem = async () => {
      ...
      if (!res.ok) {
        throw new Error(await getErrorMessage(res, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦-¦¦TÂ¦¬TÂTÌ ¦¬¦¬¦+¦¦¦¬¦¬¦¦ ¦¦¦-¦¦ ¦-¦¬TÏTÂ¦-¦¦ ¦- TÀ¦-¦-¦-TÂTÃ.'));
      }
    ...
    activateItem().catch(error => {
      setScanActivationError(toUserErrorMessage(error, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦-¦¦TÂ¦¬TÂTÌ ¦¬¦¬¦+¦¦¦¬¦¬¦¦ ¦¦¦-¦¦ ¦-¦¬TÏTÂ¦-¦¦ ¦- TÀ¦-¦-¦-TÂTÃ.'));
    });
  }, [
    fetchOrder,
  ...
  const updateTelegramStageMark = useCallback(async (columnKeys, { clear = false } = {}) => {
    ...
    try {
      ...
      const data = await parseJsonSafely(res);
      if (!res.ok) {
        throw new Error(data?.message || (clear ? '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦-¦¦¦-¦¬TÂTÌ ¦¬TÀ¦¬¦-TÏTÂ¦¬¦¦ TÍTÂ¦-¦¬¦-.' : '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦-¦¦TÂ¦¬TÂTÌ TÍTÂ¦-¦¬.'));
      }
      ...
      setStageError(toUserErrorMessage(error, clear ? '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦-¦¦¦-¦¬TÂTÌ ¦¬TÀ¦¬¦-TÏTÂ¦¬¦¦ TÍTÂ¦-¦¬¦-.' : '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦-¦¦TÂ¦¬TÂTÌ TÍTÂ¦-¦¬.'));
    } finally {
      setStageActionKey('');
    }
  }, [
  ...
  const handleOpenTelegramReadOnlyAttachment = useCallback(async (attachment, scope = 'order') => {
    ...
    if (isLinkAttachment(attachment)) {
      ...
      if (!targetUrl) {
        setTelegramActionError('¦áTÁTË¦¬¦¦¦- ¦¬TÃTÁTÂ¦-TÏ.');
        return;
      }
    ...
    const sessionToken = getActiveTelegramSessionToken();
    if (!sessionToken) {
      setTelegramActionError('¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦¬¦-¦+TÂ¦-¦¦TÀ¦+¦¬TÂTÌ Telegram-TÁ¦¦TÁTÁ¦¬TÎ ¦+¦¬TÏ ¦-TÂ¦¦TÀTËTÂ¦¬TÏ TÄ¦-¦¦¦¬¦-.');
      return;
    }
    ...
    try {
      ...
      if (isSpreadsheetAttachment(attachment)) {
        ...
        if (!res.ok) {
          setTelegramActionError(await getErrorMessage(res, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦¦TÀTËTÂTÌ Excel-TÄ¦-¦¦¦¬.'));
          return;
        }
      ...
      if (isDocxAttachment(attachment)) {
        const res = await apiFetch(fileUrl);
        if (!res.ok) {
          setTelegramActionError(await getErrorMessage(res, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦¦TÀTËTÂTÌ Word-TÄ¦-¦¦¦¬.'));
          return;
        }
        const blob = await res.blob();
        ...
        setTelegramAttachmentPreviewState({
          ...
          url: fileUrl,
          revokeUrl: false,
          html: result.value || '<p>¦ßTÃTÁTÂ¦-¦¦ ¦+¦-¦¦TÃ¦-¦¦¦-TÂ.</p>',
        });
        return;
      }
      ...
      if (!opened) {
        setTelegramActionError('¦ÑTÀ¦-TÃ¦¬¦¦TÀ ¦¬¦-¦-¦¬¦-¦¦¦¬TÀ¦-¦-¦-¦¬ ¦-TÂ¦¦TÀTËTÂ¦¬¦¦ TÄ¦-¦¦¦¬¦-. ¦à¦-¦¬TÀ¦¦TÈ¦¬TÂ¦¦ ¦-TÂ¦¦TÀTËTÂ¦¬¦¦ ¦-¦-¦-¦-¦¦ ¦-¦¦¦¬¦-¦+¦¦¦¬.');
      }
    } catch (openError) {
      setTelegramActionError(toUserErrorMessage(openError, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦¦TÀTËTÂTÌ ¦-¦¬¦-¦¦¦¦¦-¦¬¦¦.'));
    } finally {
  ...
  const addTelegramMaterialRequestPhotoItem = useCallback(async (file, source = 'gallery') => {
    ...
    const sessionToken = getActiveTelegramSessionToken();
    if (!sessionToken) {
      setMaterialRequestError('¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦¬¦-¦+TÂ¦-¦¦TÀ¦+¦¬TÂTÌ Telegram-TÁ¦¦TÁTÁ¦¬TÎ ¦+¦¬TÏ ¦¬¦-¦¦TÀTÃ¦¬¦¦¦¬ TÄ¦-TÂ¦-.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await getErrorMessage(res, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦+¦-¦-¦-¦-¦¬TÂTÌ TÄ¦-TÂ¦- ¦- ¦¬¦-TÏ¦-¦¦¦¬ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬.'));
      }
      const data = await parseJsonSafely(res);
      setOrder(data?.order || null);
      if (data?.employee) {
      ...
      setMaterialRequestError(getTelegramMaterialRequestErrorMessage(error, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦+¦-¦-¦-¦-¦¬TÂTÌ TÄ¦-TÂ¦- ¦- ¦¬¦-TÏ¦-¦¦¦¬ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬.'));
    } finally {
  ...
  const handleOpenTelegramMaterialRequestAttachment = useCallback(async (materialRequestItem, attachment) => {
    ...
    if (!fileUrl) {
      setMaterialRequestError('¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦¬¦-¦+¦¦¦-TÂ¦-¦-¦¬TÂTÌ TÁTÁTË¦¬¦¦TÃ ¦+¦¬TÏ ¦-TÂ¦¦TÀTËTÂ¦¬TÏ TÄ¦-TÂ¦-.');
      return;
    }
    ...
    try {
      ...
      const opened = openTelegramFileUrl(fileUrl, attachment.name || 'attachment');
      if (!opened) {
        setMaterialRequestError('¦ÑTÀ¦-TÃ¦¬¦¦TÀ ¦¬¦-¦-¦¬¦-¦¦¦¬TÀ¦-¦-¦-¦¬ ¦-TÂ¦¦TÀTËTÂ¦¬¦¦ TÄ¦-¦¦¦¬¦-. ¦à¦-¦¬TÀ¦¦TÈ¦¬TÂ¦¦ ¦-TÂ¦¦TÀTËTÂ¦¬¦¦ ¦-¦-¦-¦-¦¦ ¦-¦¦¦¬¦-¦+¦¦¦¬.');
      }
    } catch (error) {
      setMaterialRequestError(getTelegramMaterialRequestErrorMessage(error, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦-TÂ¦¦TÀTËTÂTÌ TÄ¦-TÂ¦- ¦¬¦-TÏ¦-¦¦¦¬ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬.'));
    } finally {
      setTelegramAttachmentOpeningKey('');
    }
  }, [
  ...
  const saveTelegramMaterialRequestName = useCallback(async (materialRequestItem) => {
    ...
    const sessionToken = getActiveTelegramSessionToken();
    if (!sessionToken) {
      setMaterialRequestError('¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦¬¦-¦+TÂ¦-¦¦TÀ¦+¦¬TÂTÌ Telegram-TÁ¦¦TÁTÁ¦¬TÎ ¦+¦¬TÏ TÁ¦-TÅTÀ¦-¦-¦¦¦-¦¬TÏ ¦-¦-¦¬¦-¦-¦-¦¬TÏ ¦¬¦-TÏ¦-¦¦¦¬.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await getErrorMessage(res, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ TÁ¦-TÅTÀ¦-¦-¦¬TÂTÌ ¦-¦-¦¬¦-¦-¦-¦¬¦¦ ¦¬¦-TÏ¦-¦¦¦¬.'));
      }
      const data = await parseJsonSafely(res);
      setOrder(data?.order || null);
      if (data?.employee) {
      ...
      setMaterialRequestError(getTelegramMaterialRequestErrorMessage(error, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ TÁ¦-TÅTÀ¦-¦-¦¬TÂTÌ ¦-¦-¦¬¦-¦-¦-¦¬¦¦ ¦¬¦-TÏ¦-¦¦¦¬.'));
    } finally {
      setMaterialRequestBusyKey('');
    }
  }, [
  ...
  const addTelegramPackageItem = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await getErrorMessage(res, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦+¦-¦-¦-¦-¦¬TÂTÌ ¦¬¦-¦¬¦¬TÆ¦¬TÎ ¦¦¦-¦-¦¬¦¬¦¦¦¦TÂ¦-TÆ¦¬¦¬.'));
      }
      const data = await parseJsonSafely(res);
      setOrder(data?.order || null);
      if (data?.employee) {
      ...
      setPackageError(getTelegramMaterialRequestErrorMessage(error, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦+¦-¦-¦-¦-¦¬TÂTÌ ¦¬¦-¦¬¦¬TÆ¦¬TÎ ¦¦¦-¦-¦¬¦¬¦¦¦¦TÂ¦-TÆ¦¬¦¬.'));
    } finally {
      setPackageBusyKey('');
    }
  }, [
  ...
  const addTelegramMaterialRequestItem = useCallback(async () => {
    ...
    if (!telegramMode || !telegramEmployee || !selectedItem?.itemId) return;
    if (!nextName) {
      setMaterialRequestError('¦Ò¦-¦¦¦+¦¬TÂ¦¦ ¦-¦-¦¬¦-¦-¦-¦¬¦¦ ¦¬¦-TÏ¦-¦¦¦¬ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await getErrorMessage(res, '¦Ý¦¦ TÃ¦+¦-¦¬¦-TÁTÌ ¦+¦-¦-¦-¦-¦¬TÂTÌ ¦¬¦-TÏ¦-¦¦TÃ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬.'));
      }
      const data = await parseJsonSafely(res);
      setOrder(data?.order || null);
      if (data?.employee) {
  ...
  if (!order) {
    return (
      <div className="card">
        <h2>¨ßÔÍ ¦×¦-¦¦¦-¦¬ ¦-¦¦ ¦-¦-¦¦¦+¦¦¦-</h2>
        <p>¦ßTÀ¦-¦-¦¦TÀTÌTÂ¦¦ TÁTÁTË¦¬¦¦TÃ ¦¬¦¬¦¬ ¦-¦-TÀ¦-TÂ¦¬TÂ¦¦TÁTÌ ¦¦ ¦-¦+¦-¦¬¦-¦¬TÁTÂTÀ¦-TÂ¦-TÀTÃ TÁ¦¬TÁTÂ¦¦¦-TË</p>
      </div>
    );
  }
  ...
  return (
    <div className={`card order-detail-card${telegramMode ? ' telegram-order-card' : ''}`}>
      <h2>{telegramMode ? `¦Ø¦¬¦+¦¦¦¬¦¬¦¦: ${selectedItem?.name || primaryItem?.name || 'òÀÔ'}` : `¨ßÓË ¦Ø¦¬¦+¦¦¦¬¦¬¦¦: ${selectedItem?.name || primaryItem?.name || 'òÀÔ'}`}</h2>
      ...
      {telegramMode ? (
        <>
            ...
            <div className="telegram-readonly-panel">
              ...
              {telegramReadOnlySection.attachments.length > 0 ? (
                <div className="telegram-readonly-files">
                  {telegramReadOnlySection.attachments.map((attachment) => {
                    const attachmentName = String(attachment?.name || '').trim() || '¦Ñ¦¦¦¬ ¦-¦-¦¬¦-¦-¦-¦¬TÏ';
                    const attachmentMeta = [
                      isLinkAttachment(attachment) ? '¦áTÁTË¦¬¦¦¦-' : '¦ä¦-¦¦¦¬',
                    ...
                    <div className="attachment-preview-toolbar-meta">
                      ...
                      <span className="attachment-preview-toolbar-size">
                        {telegramSpreadsheetPreview.sizeLabel
                          ? `${telegramSpreadsheetPreview.name || '¦ä¦-¦¦¦¬'} T¬ ${telegramSpreadsheetPreview.sizeLabel}`
                          : (telegramSpreadsheetPreview.name || '¦ä¦-¦¦¦¬')}
                      </span>
                    </div>
                    ...
                    <div className="attachment-preview-sheet-meta">
                      {(() => {
                        ...
                        return totalRows > shownRows
                          ? `¦ß¦-¦¦¦-¦¬¦-¦-TË ¦¬¦¦TÀ¦-TË¦¦ ${shownRows} ¦¬¦¬ ${totalRows} TÁTÂTÀ¦-¦¦`
                          : `¦ß¦-¦¦¦-¦¬¦-¦-¦- TÁTÂTÀ¦-¦¦: ${shownRows}`;
                      })()}
                    </div>
              ...
              {telegramAttachmentPreview && !(
                ...
                <div className="attachment-preview-panel telegram-inline-attachment-preview">
                  <div className="attachment-preview-toolbar">
                    <div className="attachment-preview-toolbar-meta">
                      <span className="attachment-preview-toolbar-kind">{telegramAttachmentPreview.kindLabel || '¦ä¦-¦¦¦¬'}</span>
                      <span className="attachment-preview-toolbar-size">
                        {telegramAttachmentPreview.sizeLabel
                          ? `${telegramAttachmentPreview.name || '¦ä¦-¦¦¦¬'} T¬ ${telegramAttachmentPreview.sizeLabel}`
                          : (telegramAttachmentPreview.name || '¦ä¦-¦¦¦¬')}
                      </span>
                    </div>
                    <div className="telegram-readonly-panel-toolbar-actions">
                  ...
                  >
                    {telegramAttachmentPreview.mode === 'image' ? (
                      <img
                        ...
                        alt={telegramAttachmentPreview.name || '¦Ø¦¬¦-¦-TÀ¦-¦¦¦¦¦-¦¬¦¦'}
                    ...
                    {telegramAttachmentPreview.mode === 'word' ? (
                      <div
                        ...
                        dangerouslySetInnerHTML={{ __html: telegramAttachmentPreview.html || '<p>¦ßTÃTÁTÂ¦-¦¦ ¦+¦-¦¦TÃ¦-¦¦¦-TÂ.</p>' }}
                      />
                    ) : null}
                    {telegramAttachmentPreview.mode === 'word-legacy' ? (
                      <div className="attachment-preview-document">
                        <p>¦ä¦-TÀ¦-¦-TÂ `.doc` ¦-¦¦ ¦-TÁ¦¦¦¦¦+¦- TÁTÂ¦-¦-¦¬¦¬TÌ¦-¦- TÀ¦¦¦-¦+¦¦TÀ¦¬TÂTÁTÏ ¦-¦-TÃTÂTÀ¦¬ Telegram Web App.</p>
                        <p>¦ä¦-¦¦¦¬ ¦+¦-TÁTÂTÃ¦¬¦¦¦- ¦¬¦- ¦¦¦-¦-¦¬¦¦¦¦ Të¦ÞTÂ¦¦TÀTËTÂTÌ ¦-TÂ¦+¦¦¦¬TÌ¦-¦-T¬ ¦-¦¦¦¬ ¦-TËTÅ¦-¦+¦- ¦¬¦¬ ¦¦¦-TÀTÂ¦-TÇ¦¦¦¬ ¦¬¦¬¦+¦¦¦¬¦¬TÏ.</p>
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <div className="telegram-stage-section">
            <div className="telegram-section-title">¦íTÂ¦-¦¬TË</div>

            {sessionLoading && (
              <div className="telegram-empty-box">
                ¦ßTÀ¦-¦-¦¦TÀTÏTÎ ¦+¦-TÁTÂTÃ¦¬ TÁ¦-TÂTÀTÃ¦+¦-¦¬¦¦¦-...
              </div>
            )}
            ...
            {!sessionLoading && !sessionError && telegramEmployee && telegramStageOptions.length === 0 && (
              <div className="telegram-empty-box">
                ¦Ô¦¬TÏ TÍTÂ¦-¦¦¦- TÁ¦-TÂTÀTÃ¦+¦-¦¬¦¦¦- ¦-¦¦ ¦-¦-TÁTÂTÀ¦-¦¦¦-TË TÍTÂ¦-¦¬TË ¦- Telegram Web App.
              </div>
            )}
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="telegram-stage-card-main">
                          ...
                          <div className="telegram-stage-card-copy">
                            ...
                            <div className="telegram-stage-card-subtitle">
                              {isMarked
                                ? `¦ßTÀ¦¬¦-TÏTÂ¦-${stageMark?.updatedBy ? ` T¬ ${stageMark.updatedBy}` : ''}${stageMark?.updatedAt ? ` T¬ ${formatDateTimeDisplay(stageMark.updatedAt)}` : ''}`
                                : '¦Þ¦¦¦¬¦+¦-¦¦TÂ ¦¬TÀ¦¬¦-TÏTÂ¦¬TÏ'}
                            </div>
                          </div>
                        </div>
                        <span className={`telegram-stage-pill ${isMarked ? 'telegram-stage-pill-complete' : 'telegram-stage-pill-pending'}`}>
                          {isMarked ? '¦ßTÀ¦¬¦-TÏTÂ¦-' : '¦Þ¦¦¦¬¦+¦-¦¦TÂ'}
                        </span>
                      </div>
                      <div className="telegram-stage-card-actions">
                        ...
                        >
                          {isBusy
                            ? (isMarked ? '¦ÞTÂ¦-¦¦¦-TÏTÎ...' : '¦á¦-TÅTÀ¦-¦-TÏTÎ...')
                            : (isMarked ? '¦ÞTÂ¦-¦¦¦-¦¬TÂTÌ ¦¬TÀ¦¬¦-TÏTÂ¦¬¦¦' : '¦ßTÀ¦¬¦-TÏTÂTÌ TÍTÂ¦-¦¬')}
                        </button>
                      </div>
                    </div>
                  );
          ...
          {(sessionLoading || sessionError || canManagePackage) && (
            <div className="telegram-package-section">
              <div className="telegram-section-title">¦ã¦¦¦-¦-¦¬¦¬¦¦¦¦TÂ¦-¦-¦-¦-¦-</div>

              {sessionLoading && (
                <div className="telegram-empty-box">
                  ¦ßTÀ¦-¦-¦¦TÀTÏTÎ ¦+¦-TÁTÂTÃ¦¬ ¦¦ ¦¦¦-¦-¦¬¦¬¦¦¦¦TÂ¦-TÆ¦¬¦¬...
                </div>
              )}
              ...
              {canManagePackage && (
                <>
                  <div className="telegram-package-meta">
                    ¦Ú¦-¦-¦¬¦¬¦¦¦¦TÂ¦-TÆ¦¬TÏ: {packageStats.completed}/{packageStats.total || 0}
                  </div>
                  ...
                  <div className="telegram-package-input-row">
                    <input
                      ...
                      placeholder="¦Ô¦-¦-¦-¦-¦¬TÂTÌ ¦¬¦-¦¬¦¬TÆ¦¬TÎ ¦¦¦-¦-¦¬¦¬¦¦¦¦TÂ¦-TÆ¦¬¦¬"
                    ...
                    <button
                      ...
                      disabled={packageBusyKey === 'add'}
                    >
                      {packageBusyKey === 'add' ? '¦Ô¦-¦-¦-¦-¦¬TÏTÎ...' : '¦Ô¦-¦-¦-¦-¦¬TÂTÌ'}
                    </button>
                  </div>
                          ...
                          >
                            ...
                            <div className="telegram-stage-card-actions">
                              ...
                              >
                                {isBusy
                                  ? '¦á¦-TÅTÀ¦-¦-TÏTÎ...'
                                  : (packageItem.isCompleted ? '¦á¦-TÏTÂTÌ ¦-TÂ¦-¦¦TÂ¦¦TÃ' : '¦ÞTÂ¦-¦¦TÂ¦¬TÂTÌ')}
                              </button>
                            </div>
                          </div>
                        );
          ...
          {(sessionLoading || sessionError || canManageMaterialRequests) && (
            <div className="telegram-package-section telegram-material-request-section">
              <div className="telegram-section-title">¦×¦-TÏ¦-¦¦¦¬ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬</div>

              {sessionLoading && (
                <div className="telegram-empty-box">
                  ¦ßTÀ¦-¦-¦¦TÀTÏTÎ ¦+¦-TÁTÂTÃ¦¬ ¦¦ ¦¬¦-TÏ¦-¦¦¦-¦- ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬...
                </div>
              )}
              ...
              {canManageMaterialRequests && (
                <>
                  <div className="telegram-package-meta telegram-material-request-meta">
                    ¦Ò¦-¦¦¦-TË¦¦ ¦¬¦-TÏ¦-¦¦¦¬: ¦+¦-¦-¦-¦-¦¬¦¦¦-¦- {materialRequestStats.total}
                  </div>
                  ...
                  <div className="telegram-package-input-row telegram-material-request-input-row">
                    <input
                      ...
                      placeholder="¦Ô¦-¦-¦-¦-¦¬TÂTÌ ¦¬¦-TÏ¦-¦¦TÃ, ¦-¦-¦¬TÀ¦¬¦-¦¦TÀ TÁ¦-¦¦TÀ¦¬¦-"
                    ...
                    <button
                      ...
                      disabled={materialRequestBusyKey === 'add'}
                    >
                      {materialRequestBusyKey === 'add' ? '¦Ô¦-¦-¦-¦-¦¬TÏTÎ...' : '¦Ô¦-¦-¦-¦-¦¬TÂTÌ'}
                    </button>
                  </div>
                  ...
                  <div className="telegram-material-request-add-photo-row">
                    <label className="btn btn-primary telegram-material-request-upload-label">
                      ...
                      />
                      <span>
                        {materialRequestBusyKey === 'add-photo:gallery' ? '¦×¦-¦¦TÀTÃ¦¦¦-TÎ...' : '¦Ô¦-¦-¦-¦-¦¬TÂTÌ ¦¬¦¬ ¦¦¦-¦¬¦¦TÀ¦¦¦¬'}
                      </span>
                    </label>
                  </div>
                                  ...
                                  <>
                                    <div className="telegram-stage-card-top">
                                      <div className="telegram-stage-card-main">
                                        ...
                                        <div className="telegram-stage-card-copy">
                                          ...
                                          <div className="telegram-stage-card-subtitle">
                                            {attachment?.uploadedAt ? `¦Ô¦-¦-¦-¦-¦¬¦¦¦-¦- T¬ ${formatDateTimeDisplay(attachment.uploadedAt)}` : '¦Ô¦-¦-¦-¦-¦¬¦¦¦-¦- ¦- ¦¬¦-TÏ¦-¦¦¦¬ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬'}
                                          </div>
                                        </div>
                                      </div>
                                    ...
                                    {photoUrl ? (
                                      ...
                                      >
                                        <img
                                          src={photoUrl}
                                          alt={attachment?.name || '¦ä¦-TÂ¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦-'}
                                    ...
                                    {isInlinePreviewOpen ? (
                                      <div className="telegram-material-request-inline-preview">
                                        <img
                                          src={telegramAttachmentPreview.url}
                                          alt={telegramAttachmentPreview.name || '¦ä¦-TÂ¦-'}
        ...
        <div className="table-scroll">
          <table>
            <tbody>
              <tr><td><strong>¦Ý¦-¦-¦¦TÀ ¦¬¦-¦¦¦-¦¬¦-</strong></td><td>{order.orderNumber || 'òÀÔ'}</td></tr>
              <tr><td><strong>¦×¦-¦¦¦-¦¬TÇ¦¬¦¦</strong></td><td>{order.customer || 'òÀÔ'}</td></tr>
              <tr><td><strong>¦Ø¦¬¦+¦¦¦¬¦¬¦¦</strong></td><td>{selectedItem?.name || primaryItem?.name || 'òÀÔ'}</td></tr>
              <tr><td><strong>òÄÖ ¦¬¦¬¦+¦¦¦¬¦¬TÏ ¦- ¦¬¦-¦¦¦-¦¬¦¦</strong></td><td>{selectedItem?.itemNumber || 'òÀÔ'}</td></tr>
              <tr><td><strong>¦ß¦-¦-¦¦TÉ¦¦¦-¦¬¦¦</strong></td><td>{selectedItem?.room || 'òÀÔ'}</td></tr>
              <tr><td><strong>òÄÖ ¦¬¦-¦-¦¦TÉ¦¦¦-¦¬TÏ</strong></td><td>{selectedItem?.roomNumber || 'òÀÔ'}</td></tr>
              <tr><td><strong>¦Ú¦-¦¬-¦-¦- ¦¬¦¬¦+¦¦¦¬¦¬¦¦</strong></td><td>{selectedItem?.quantity || primaryItem?.quantity || 1}</td></tr>
              <tr><td><strong>¦Ü¦-TÂ¦¦TÀ¦¬¦-¦¬</strong></td><td>{selectedItem?.material || primaryItem?.material || 'òÀÔ'}</td></tr>
              <tr><td><strong>¦Ú¦-¦-¦¬¦¬¦¦¦¦TÂ¦-TÆ¦¬TÏ</strong></td><td>{selectedItem?.packageName || 'òÀÔ'}</td></tr>
              <tr><td><strong>¦×¦-TÏ¦-¦¦¦¬ ¦-¦- TÀ¦-TÁTÅ¦-¦+¦-¦¬¦¦¦¬</strong></td><td>{materialRequestStats.total > 0 ? `${materialRequestStats.completed}/${materialRequestStats.total}` : 'òÀÔ'}</td></tr>
              <tr><td><strong>¦ßTÀ¦¬¦-¦¦TÇ¦-¦-¦¬TÏ</strong></td><td>{selectedItem?.notes || primaryItem?.notes || 'òÀÔ'}</td></tr>
              <tr><td><strong>¦Ô¦-TÂ¦- ¦¬¦-¦¦¦-¦¬¦-</strong></td><td>{formatDateDisplay(order.orderDate)}</td></tr>
              <tr><td><strong>¦Ý¦-TÇ¦-¦¬¦- ¦¬¦¬¦¦¦-TÂ¦-¦-¦¬¦¦¦-¦¬TÏ</strong></td><td>{formatDateDisplay(order.startDate)}</td></tr>
              <tr><td><strong>¦Þ¦¦¦-¦-TÇ¦-¦-¦¬¦¦ ¦¬¦¬¦¦¦-TÂ¦-¦-¦¬¦¦¦-¦¬TÏ</strong></td><td>{formatDateDisplay(order.endDate)}</td></tr>
              <tr><td><strong>¦ÒTÀ¦¦¦-TÏ ¦¬¦¬¦¦¦-TÂ¦-¦-¦¬¦¦¦-¦¬TÏ</strong></td><td>{calcDuration(order.startDate, order.endDate)}</td></tr>
              <tr><td><strong>¦áTÂ¦-TÂTÃTÁ</strong></td><td><span className={statusMeta.className}>{statusMeta.label}</span></td></tr>
            </tbody>
          </table>
        </div>
      )}
